...
surfaceNets :: Voxel -> Seq (Seq Double)
surfaceNets (dat, (nx,ny,nz), ((xmin,xmax),(ymin,ymax),(zmin,zmax))) = 
  loop2 0 1 S.empty
  where
  loop2 :: Int -> Int -> Seq (Seq Double) -> Seq (Seq Double)
  loop2 x2 b vs | x2 == nz-1 = vs
                | otherwise = 
                  loop2 (x2+1) (1-b) 
                        (loop1 0 x2 (1 + (nx + 1) * (1 + b * (ny + 1))) vs)
  loop1 :: Int -> Int -> Int -> Seq (Seq Double) -> Seq (Seq Double)
  loop1 x1 x2 m vs | x1 == ny-1 = vs
                   | otherwise = 
                     loop1 (x1+1) x2 (m+2) (loop0 0 x1 x2 m vs)
  loop0 x0 x1 x2 m vs | x0 == nx-1 = vs
                      | otherwise = 
                        loop0 (x0+1) x1 x2 (m+1) (f1 vs x0 x1 x2)
  f1 vs x0 x1 x2 = vsv 
    where
    grid = [dat !! (x0 + i + (x1 + j) * nx + (x2 + k) * nx * ny) | 
            k <- [0,1], j <- [0,1], i <- [0,1]]
    is = imap (\i d -> if d<0 then 0::Int else shiftL (1::Int) i) grid
    mask = foldr (.|.) 0 is
    vsv = if mask == 0 || mask == 255
      then
        vs
      else
